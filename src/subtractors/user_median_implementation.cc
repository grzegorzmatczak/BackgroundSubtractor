...
Subtractor::imgMedian::imgMedian(unsigned char R_temp)
{
  R = R_temp;
}
...
void Subtractor::imgMedian::InitMedian(cv::Mat &img)
{
  ...
  for (unsigned int y = 0; y < height; y++)
  {
    for (unsigned int x = 0; x < width; x++)
    {
      ...
      {
        V[i][c] =
            static_cast<unsigned char>(img.at<unsigned char>(cv::Point(static_cast<int>(x), static_cast<int>(y))));
        NEXT[i][c] = static_cast<unsigned char>(c + 1);
        PREV[i][c] = static_cast<unsigned char>(c - 1);
      }

      V[i][0] = (img.at<unsigned char>(cv::Point(static_cast<int>(x), static_cast<int>(y))));
...
{
  int i = 0;
  for (std::vector<unsigned char>::iterator it = V[0].begin(); it != V[0].end(); it++)
  {
...
void Subtractor::imgMedian::ShowMedianNode(unsigned int temp)
{
  ...
  for (std::vector<unsigned char>::iterator it = V[temp].begin(); it != V[temp].end(); it++)
  {
    std::cout << static_cast<int>(*it) << '\t';
  }
  std::cout << "\n";
  for (std::vector<unsigned char>::iterator it = NEXT[temp].begin(); it != NEXT[temp].end(); it++)
  {
    std::cout << static_cast<int>(*it) << '\t';
  }
  std::cout << "\n";
  for (std::vector<unsigned char>::iterator it = PREV[temp].begin(); it != PREV[temp].end(); it++)
  {
    std::cout << static_cast<int>(*it) << '\t';
  }
  std::cout << "\n";
...
void Subtractor::imgMedian::getMedian(cv::Mat &img)
{
  ...
  for (y = 0; y < height; y++)
  {
    for (x = 0; x < width; x++)
    {
      ...
      img.at<unsigned char>(cv::Point(static_cast<int>(x), static_cast<int>(y))) = V[i][insertRow];
      V[i][0] = V[i][insertRow];
    }
  }
}
...
cv::Mat Subtractor::imgMedian::getMedian()
{
  ...
  for (y = 0; y < height; y++)
  {
    for (x = 0; x < width; x++)
    {
      ...
      localCopy.at<unsigned char>(cv::Point(static_cast<int>(x), static_cast<int>(y))) = V[i][insertRow];
      V[i][0] = V[i][insertRow];
    }
  }
  return localCopy;
...
void Subtractor::imgMedian::UnitTestInit(cv::Mat M, int R, vector<unsigned char> V1, vector<unsigned char> N1,
                                         vector<unsigned char> P1, unsigned int interator)
{
  InitMedian(M);
  ShowIterator();
...
void Subtractor::imgMedian::UnitTestCheck(vector<unsigned char> V1, vector<unsigned char> N1, vector<unsigned char> P1)
{
  unsigned int i = 0;
  for (std::vector<unsigned char>::iterator it = V[0].begin(); it != V[0].end(); ++it)
  {
  ...
  i = 0;
  for (std::vector<unsigned char>::iterator it = NEXT[0].begin(); it != NEXT[0].end(); ++it)
  {
    if (NEXT[0][i] != N1[i])
    {
  ...
  i = 0;
  for (std::vector<unsigned char>::iterator it = PREV[0].begin(); it != PREV[0].end(); ++it)
  {
    if (PREV[0][i] != P1[i])
    {
